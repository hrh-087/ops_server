...
func StopGameServer(projectId uint, hostId uint, gameServerIds []uint) (output string, err error) {
	...
	for _, gameServer := range gameServerList {
		// 脚本命令 渠道名称 游戏服目录
		command := fmt.Sprintf("bash %s %s %s_%d", filepath.Join(global.OPS_CONFIG.Game.GameScriptAutoPath, "stop_game.sh"), gameServer.Platform.PlatformCode, gameServer.GameType.Code, gameServer.Vmid)
